...
namespace ChuckHill2
{
    ...
    public class PerfTimer
    {
        ...
        public PerfTimer() { }
        public PerfTimer(Log log) { _log = log; }
        ...
        /// <summary>
        /// Print formatted current elapsed time message to Verbose Event Log. 
        ...
        {
            if (_log == null || !_log.IsVerboseEnabled) return;
            _log.Verbose("Performance: " + format + string.Format(": {0} ms", this.Count), args);
        }

        /// <summary>
        /// Print current elapsed time message to debug output window. 
        /// Does not stop or pause the timer.
        /// This method does nothing if DBG.Enabled is false.
        /// </summary>
        /// <param name="msg">string message to print with elapsed time appended"</param>
        public void Print(string msg)
        {
            if (!DBG.Enabled) return;
            DBG.WriteLine("{0}: {1} ms", msg, this.Count);
        }
        ...
        {
            if (!DBG.Enabled) return;
            DBG.WriteLine(format + string.Format(": {0} ms", this.Count), args);
        }
    }
}
